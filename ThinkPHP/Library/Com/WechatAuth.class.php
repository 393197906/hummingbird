...
class WechatAuth
{
    ...
    /**
     *用户同意授权，获取code（获取授权页面）getAccessToken（‘code’,getRequestCodeURL($redirect_uri)）
     * appid 公众号的唯一标识（必须）
     * @param $redirect_uri 授权后重定向的回调链接地址，请使用urlencode对链接进行处理
     * @response_type 返回类型，请填写code（code作为换取access_token的票据，每次用户授权带上的code将不一样，
    ...
    /**
     * @param string $type 客户端 网页授权
     *
     * @param null $code 网页授权获取用户基本信息
     ...
     * @param $type 媒体资源类型，分别有图片（image）、语音（voice）、视频（video）和缩略图（thumb，主要用于视频与音乐格式的缩略图）
    ...
    public function mediaUpload($filename, $type)
    {
        ...
        return $this->api('media/upload', $data, 'POST', $type, false);
    }
    ...
    public function mediaGet($media_id)
    {
        ...
        return $url . http_build_query($param);//http_build_query — 生成 URL-encode 之后的请求字符串 .access_token=accessToken&media_id=MEDIA_ID
    }
    ...
    public function materialAddNews($articles)
    {
        $data = array(
            ...
            'thumb_media_id' =>	$articles['thumb_media_id'],// 图文消息的封面图片素材id（必须是永久mediaID）
            'author'=>$articles['author'],//作者
            'digest'=>$articles['digest'],//图文消息的摘要，仅有单图文消息才有摘要，多图文此处为空
            'show_cover_pic'=>$articles['show_cover_pic'],//是否显示封面，0为false，即不显示，1为true，即显示
            'content'=>$articles['content'],//图文消息的具体内容，支持HTML标签，必须少于2万字符，小于1M，且此处会去除JS
            'ontent_source_url'=>$articles['ontent_source_url'],//图文消息的原文地址，即点击“阅读原文”后的URL
        );
        return $this->api('material/add_news', $data, 'POST', '', false);
    }

    /**
     *  新增永久素材之新增永久图文素材
     * 请求方式: POSThttps://api.weixin.qq.com/cgi-bin/media/uploadimg?access_token=ACCESS_TOKEN
     *  上传图文消息内的图片获取URL
    ...
    public function mediaUploadimg($filename)
    {
        ...
        return $this->api('media/uploadimg', $data, 'POST', '', false);
    }
    //获取永久素材
    //在新增了永久素材后，开发者可以根据media_id来获取永久素材，需要时也可保存到本地。
//https://api.weixin.qq.com/cgi-bin/material/get_material?access_token=ACCESS_TOKEN

    public function materialGetMaterial($media_id)
    {
        $param = array(
            'media_id' => $media_id,
        );
        return $this->api('material/get_material', '', 'GET', $param, false);
    }
     ...
     * @return array https://api.weixin.qq.com/cgi-bin/menu/create?access_token=ACCESS_TOKEN
     */
     ...
     * @param  array $data POST请求数据 格式为数组
     * @param  string $method 请求方式
     * @param  array $param GET/POST参数
    ...
    protected function api($name, $data , $method = 'POST', $param, $json = true)
    {
        ...
        if ($json && !empty($data)) {
            ...
            $data = urldecode(json_encode($data));//json_encode array ===>  json数据 ;urldecode — 解码已编码的 URL 字符串
        }

        $data = self::http($url, $params, $data, $method);

        return json_decode($data, true);// JSON ===> array数组
    }

    /**
     * Send a POST requst using cURL
     * @param  string $url 请求URL
     * @param  array $param 额外的参数 注意写法$params = array('access_token' => $this->accessToken);=》 access_token=accessToken&&..
    ...
    protected static function http($url, $param, $data = '', $method = 'GET')
    {
        ...
        $data = curl_exec($ch);//功时返回 TRUE， 或者在失败时返回 FALSE。 然而，如果 CURLOPT_RETURNTRANSFER选项被设置，函数执行成功时会返回执行的结果，失败时返回 FALSE 。
        $error = curl_error($ch);
        curl_close($ch);
